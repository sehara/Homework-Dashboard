...
function renderCourseCards() {
    ...
    // Ensure ALL courses from courseInfo get a card, even if they have no courseData entries
    if (typeof courseInfo !== 'undefined') {
        Object.keys(courseInfo).forEach(courseName => {
            if (!courseStats[courseName]) {
                courseStats[courseName] = {
                    total: 0, remaining: 0, totalHours: 0,
                    remainingHours: 0, scheduled: 0, unscheduled: 0
                };
            }
        });
    }
    ...
    Object.keys(courseStats).forEach(courseName => {
        ...
        if (stats.total === 0) {
            // No data yet for this course (e.g. professor hasn't posted)
        ...
        if (stats.total === 0) {
            // Placeholder card for courses with no data yet
            const info = typeof courseInfo !== 'undefined' ? courseInfo[courseName] : null;
